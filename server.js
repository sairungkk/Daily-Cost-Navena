...
function backupExcelFile() {
  ...
  try {
    ...
    // เก็บไฟล์สำรองล่าสุดไม่เกิน MAX_BACKUPS ไฟล์ (ชื่อไฟล์มี timestamp เรียงตามตัวอักษร = เรียงตามเวลา)
...
// บันทึกไฟล์ Excel อย่างปลอดภัย: เขียนลงไฟล์ชั่วคราวก่อนแล้วค่อยแทนที่ไฟล์จริง
// กันไฟล์เสียหายเมื่อเขียนค้างกลางทาง และแจ้งเตือนชัดเจนเมื่อไฟล์ถูกเปิดล็อกอยู่ในโปรแกรม Excel
async function saveWorkbook(workbook) {
  ...
  try {
    ...
    try { fs.unlinkSync(tmpFile); } catch (cleanupErr) { /* ลบไฟล์ชั่วคราวไม่ได้ ข้ามได้ */ }
...
app.post('/api/import-excel', requireAuth('admin'), upload.single('file'), async (req, res) => {
  try {
    ...
    const mealMapAggregator = {}; // เก็บจัดกลุ่มสำหรับ Outlet_Summary

    // 1. วนลูปอ่านข้อมูลทีละชีทจากไฟล์ที่อัปโหลด
    uploadedWorkbook.worksheets.forEach(sheet => {
      ...
      if (sheetName.includes('purchases') || sheetName.includes('issue')) {
        sheet.eachRow((row, rowNum) => {
          ...
          const p = colMap.purchase ? Number(row.getCell(colMap.purchase).value) || 0 : 0;
          const i = colMap.issue ? Number(row.getCell(colMap.issue).value) || 0 : 0;
          ...
          if (d && out && purSheet) {
            ...
            if (p > 0 || i > 0) {
              purSheet.addRow({ date: d, outlet: out, purchase: p, issue: i, timestamp: currentTimestamp });
              totalImportedRows++;
            }
          }
        });
      }
      // --- กรณีเป็นชีท Cost_Adjustments ---
      else if (sheetName.includes('adjustment') || sheetName.includes('transfer')) {
        sheet.eachRow((row, rowNum) => {
          if (rowNum === 1) return;
          const d = colMap.date ? getCellDateStr(row.getCell(colMap.date).value) : '';
          const out = colMap.outlet ? String(row.getCell(colMap.outlet).value || '').trim() : '';
          const tIn = colMap.transferIn ? Number(row.getCell(colMap.transferIn).value) || 0 : 0;
          const tOut = colMap.transferOut ? Number(row.getCell(colMap.transferOut).value) || 0 : 0;
          const cr = colMap.crCost ? Number(row.getCell(colMap.crCost).value) || 0 : 0;
          ...
          if (d && out && costAdjSheet) {
            ...
            if (tIn > 0 || tOut > 0 || cr > 0) {
              costAdjSheet.addRow({ date: d, outlet: out, transferIn: tIn, transferOut: tOut, crCost: cr, timestamp: currentTimestamp });
              totalImportedRows++;
            }
          }
        });
      }
      // --- กรณีเป็นชีท Hotel_Stats ---
      ...
      // --- กรณีเป็นชีท Data / Outlets ปกติ ---
        ...
        sheet.eachRow((row, rowNum) => {
          ...
          if (!importedDateOutlets.has(pairKey)) {
            ...
            // ล้างข้อมูลเก่าของ Date & Outlet นั้นๆ
            if (dataSheet) {
          ...
          // รวมยอดสำหรับ Outlet_Summary
          const sumKey = `${d}___${out}___${meal}`;
          if (!mealMapAggregator[sumKey]) {
            mealMapAggregator[sumKey] = { date: d, outlet: out, meal: meal, foodRev: 0, bevRev: 0, cover: 0, foodCost: 0, bevCost: 0 };
          }
          if (cat === 'RV-Food') {
    ...
    // 3. จัดเรียงข้อมูลและอัปเดตชีทสูตรสรุป
...
app.post('/api/save-excel', requireAuth('admin'), async (req, res) => {
  try {
    ...
    // บันทึกเฉพาะเมื่อมีตัวเลขสถิติ > 0 หรือเป็นการอัปเดตสภาพอากาศบนแถวเดิม
    // (กันการสร้างแถวสถิติเป็นศูนย์ตอนบันทึก Canteen เพราะ weather ไม่มีทางว่าง)
    const hasStatsNumbers = Number(roomAvailable) > 0 || Number(roomSold) > 0 || Number(comHuRoom) > 0 || Number(totalGuest) > 0;
    if (statsSheet && (hasStatsNumbers || (weather && existingRow))) {
      if (existingRow) {
...
app.post('/api/seed-random', requireAuth('admin'), async (req, res) => {
  try {
    ...
    for (let i = 30; i >= 0; i--) {
      ...
      const dateStr = toLocalDateStr(d); // ใช้วันที่ตามเวลาเครื่อง กัน toISOString เพี้ยนช่วงเที่ยงคืน-06:59 ของเวลาไทย
  ...
  try {
    ...
    Object.keys(aggregator).forEach(key => {
      ...
      if (mode === 'allocated' && item.rawTotalCost > 0) {
        const fShare = item.foodCost / item.rawTotalCost;
        const bShare = item.bevCost / item.rawTotalCost;
        const wShare = item.wineCost / item.rawTotalCost;
        const oShare = item.otherCost / item.rawTotalCost;

        foodCostFinal += (adjShare * fShare);
        bevCostFinal += (adjShare * bShare);
        wineCostFinal += (adjShare * wShare);
        otherCostFinal += (adjShare * oShare);
      } else if (mode === 'allocated' && outSummary.totalRawCost > 0) {
        const share = item.rawTotalCost / outSummary.totalRawCost;
        foodCostFinal += (outSummary.adjustments * share * 0.6);
        bevCostFinal += (outSummary.adjustments * share * 0.2);
        wineCostFinal += (outSummary.adjustments * share * 0.15);
        otherCostFinal += (outSummary.adjustments * share * 0.05);
      }

      const totalCostFinal = foodCostFinal + bevCostFinal + wineCostFinal + otherCostFinal;
...
// Error handler กลาง เช่น กรณีอัปโหลดไฟล์เกินขนาดที่ multer กำหนด
